...
设备升级到固件 `1.4.1` 后，网页会显示“投喂参数”面板和“测试当前舵机”按钮：

1. 先不要安装粮仓负载，打开“请求状态”确认固件版本为 `1.4.1`。
2. 默认模式是“360°连续旋转”：设置一圈耗时、转动角度和投喂方向；先测试180°，再测试360°。
3. 如果使用180°定位舵机，在网页切换模式后再设置关闭位置和投喂位置，范围0～180°。
...
| 计划或参数保存失败 | 设备是否在线、固件是否已升级到 1.4.1 |

...
设备升级到固件 `1.5.3` 后，网页会显示完整动作序列和“测试当前舵机”按钮：

1. 先不要安装粮仓负载，打开“请求状态”确认固件版本为 `1.5.3`。
2. 180° SG90 设置关闭位置、方向、单程角度、去程时间、到位停留和回程时间。机械转子量杯建议先从关闭10°、打开170°开始，再按实际安装位置微调。
3. 如果切换到360°连续舵机，再设置方向、估算转动角度和一圈耗时。例如90°且一圈4000毫秒，单程约运行1000毫秒；再设置到位停留和是否反转回位。
4. 点击“保存到投喂器”，等待页面显示“已保存到投喂器”；然后点击“测试当前舵机”。测试不会计入投喂次数，适合先观察 SG90 是否从关闭角度转到投喂角度再回位。
4. 设置两次投喂最短间隔和每日最大次数。
5. 点击“保存到投喂器”，等待设备确认；网页关闭或设备断电后参数仍保留。
6. 连续舵机若停止后仍慢慢转，展开“连续舵机方向/停止校准”，调整停止脉宽；若方向相反，切换投喂方向。
...
3. 选择执行星期；每次到点只执行一次完整动作。
4. 保持计划启用，点击“保存到投喂器”。
5. 等待“定时计划已保存到投喂器”的提示。
6. 点击“请求状态”，确认计划仍显示在页面中。
7. 完全关闭网页或关闭手机屏幕。
8. 到达设定时间后观察舵机是否自动执行。
9. 重新打开网页，检查最近投喂时间、来源和 24 小时执行次数。
...
| 计划或参数保存失败 | 设备是否在线、固件是否已升级到 1.5.3 |
| 舵机测试无动作 | 检查状态中的舵机模式和角度，确认点击测试后收到确认；再检查 SG90 红线5V、棕线GND、橙线D1，并确认外部电源与 NodeMCU 共地 |

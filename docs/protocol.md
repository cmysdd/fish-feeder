...
```text
舵机模式,起始角度,目标角度,连续转动角度,一圈耗时毫秒,正转脉宽,反转脉宽,停止脉宽,连续投喂方向,连续是否回位,180去程毫秒,到位停留毫秒,180回程毫秒,最短间隔秒数,每日最大份量,每日最大次数
```

例如 `0,90,180,90,4000,1700,1300,1500,0,1,1000,1000,1000,60,12,8`。模式 `1` 为360°连续旋转，按“连续转动角度 ÷ 360 × 一圈耗时”计算单程时间；回位字段为 `1` 时，在到位停留后反向运行同样时间。模式 `0` 为180°定位舵机，网页根据起始位置、方向和相对角度计算目标角度，固件按去程时间平滑移动，到位停留后再按回程时间返回。新版仍兼容旧版13字段和12字段参数，缺少的动作时间使用默认值。
...
```json
{
  ...
  "positional_move_ms": 1000,
  "action_pause_ms": 1000,
  "positional_return_ms": 1000,
  "last_error": "",
